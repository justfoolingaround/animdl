...
PartialEpisode = namedtuple(
    'AFLEpisode', [
        'number', 'title', 'content_type', 'aired_date'])
...
def get_filler_list(
    ...
    if canon:
        initial_xpath.append(
            "//tr[@class='manga_canon even'] | //tr[@class='manga_canon odd'] | //tr[@class='anime_canon even'] | //tr[@class='anime_canon odd']")

    if mixed_canon:
        initial_xpath.append(
            "//tr[@class='mixed_canon/filler even'] | //tr[@class='mixed_canon/filler odd']")

    if fillers:
        initial_xpath.append(
            "//tr[@class='filler even'] | //tr[@class='filler odd']")
...
def get_using_xpath(session, afl_url, xpath):
    ...
    return [
        PartialEpisode(
            int(
                tr.xpath('td[@class="Number"]')[0].text),
            tr.xpath('td[@class="Title"]/a')[0].text.strip(),
            tr.xpath('td[@class="Type"]/span')[0].text.strip(),
            tr.xpath('td[@class="Date"]')[0].text.strip()) for tr in parsed.xpath(xpath)]
