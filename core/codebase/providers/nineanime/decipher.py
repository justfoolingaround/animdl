...
def decipher(encrypted_url: str):
    ...
    for index, character in enumerate(s2, 1):
        ...
        if index and not (index % 4):
            decrypted, crypto = decrypted + \
                chr((0xff0000 & crypto) >> 16) + chr((0xff00 & crypto) >> 8) + chr(0xff & crypto), 0
    ...
    for byte_index in range(0x100):
        xcrypto = (xcrypto + mapper.get(byte_index) +
                   ord(s1[byte_index % len(s1)])) % 0x100
    ...
    for character in decrypted:
        ...
        appendlet = chr(ord(character) ^ mapper[(
            mapper[xcryptoy] + mapper[xcryptoz]) % 0x100])
