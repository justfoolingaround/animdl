...
CONTENT_SLUG_RE = construct_site_based_regex(ANIMTIME, extra_regex=r'/title/([^/?&]+)')
...
def fetcher(session, url, check):
    ...
    main = get_first_item(scripts, lambda e: e.get('src', '').startswith('main')).get('src')
    ...
    episodes = int(re.search(r'zd.*?\[tm\.{}\]=(\d+)'.format(re.escape(anime.group(1))), content).group(1))
    constructor, end = re.search('\[tm\.{}\]=function\(t\){{return"([^"]+)\"\+t\+\"([^"]+)'.format(re.escape(anime.group(1))), content).groups()
