...
from core import *
from core.twistmoe.stream_url import DL_HEADERS, TWIST_URL_RE
...
if __name__ == '__main__':
    ...
    internal_download(os.getenv('FOLDER_NAME', 'AnimDL Download'), client.fetch_episodes_using_animix(start=int(os.getenv('START') or 0) or None, end=int(os.getenv('END') or 0) or None, offset=int(os.getenv('ANIME_FILLER_LIST_OFFSET', 0) or 0), canon=bool(int(os.getenv('CANONS') or 0)), fillers=bool(int(os.getenv('FILLERS') or 0)), mixed_canon=bool(int(os.getenv('MIXED_CANONS_AND_FILLERS') or 0))), headers=DL_HEADERS if TWIST_URL_RE.match(client.url) else {})