...
def download(session, logger, content_dir, outfile_name, stream_urls, quality, **kwargs):
    ...
    for download_data in iter(downloadable_content):
        if "further_extraction" in download_data:
            try:
                further_status, further_yield = download(
                    session, logger, content_dir, outfile_name, further_extraction(session, download_data), quality, **kwargs)
                if further_status:
                    return further_status, further_yield
                continue
            except Exception as e:
                logger.critical(
                    "Could not extract from the stream due to {!r}. falling back to other streams.".format(e))
                continue
        ...
        try:
            ...
            logger.critical(
                "Oops, due to {!r}, this stream has been rendered unable to download.".format(e))
