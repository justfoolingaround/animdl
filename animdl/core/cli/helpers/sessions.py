...
def update_session_file(session_file, session_dict):
    ...
    with open(session_file, 'w') as sw:
        json.dump([s for s in sessions if (s.get('identifier', '')).lower(
        ) != session_dict.get('identifier', '').lower()] + [session_dict], sw, indent=4)
...
def describe_session_dict(session_dict):
    ...
    if session_dict.get('type') == 'download':
        initial += " [Downloading" + \
            " using IDM]" if session_dict.get('idm') else ']'

    end = session_dict.get('end')
    initial += " [{}/{}]".format(session_dict.get('start'),
                                 end if isinstance(end, int) else '?')

    afl = session_dict.get('afl')

    if afl.get('url'):
        initial += " [AFL: {url} with an offset of {offset} and filters set as filler: {}, canon: {}, mixed filler/canon: {}".format_map(
            afl)
