...
def search_9anime(session, query):
    ...
    nineanime_results = session.get(NINEANIME_URL_SEARCH, params={
                                    'keyword': query}, headers={'cookie': 'waf_cv=%s' % waf_token})
...
def search_animepahe(session, query):
    ...
    animepahe_results = session.get(ANIMEPAHE_URL_SEARCH_AJAX, params={
                                    'q': query, 'm': 'search'})
...
def search_animixplay(session, query):
    parsed = htmlparser.fromstring(session.get(
        GOGOANIME_URL_SEARCH, params={'keyword': query}).text)
...
def search_twist(session, query):
    content = session.get(TWIST_URL_CONTENT_API, headers={
...
def search_crunchyroll(session, query):
    content = json.loads(session.get(
        CRUNCHYROLL + "ajax/?req=RpcApiSearch_GetSearchCandidates").text.strip('*/\n -secur'))
...
def search_tenshi(session, query):
    ...
    ajax_content = session.post(TENSHI_URL_SEARCH_POST, data={'q': query}, headers={
                                'x-requested-with': 'XMLHttpRequest', 'x-csrf-token': token, 'referer': 'https://tenshi.moe/', 'cookie': 'tenshimoe_session={}'.format(session_id)})
