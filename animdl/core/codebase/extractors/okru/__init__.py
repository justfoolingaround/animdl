...
def extract(session, url, **opts):
    ...
    metadata = htmlparser.fromstring(
        response.text).cssselect('div[data-module="OKVideo"]')

    if not metadata:
        return []

    data_opts = json.loads(json.loads(unescape(metadata[0].get(
        'data-options'))).get('flashvars', {}).get('metadata'))
