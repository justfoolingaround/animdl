...
def extract(session, url, **opts):
    content = htmlparser.fromstring(
        session.get(url.replace('/e/', '/d/')).text)
    ...
    def fast_yield():
        ...
        for link_column, quality_column in zip(genexp, genexp):
            ...
            content_id, mode, content_hash = DOWNLOAD_CONTENT.search(
                links[0].get('onclick')).groups()

            download_link = False
            while not download_link:
                download_page = htmlparser.fromstring(session.get('https://sbplay1.com/dl', params={
                                                      'op': 'download_orig', 'id': content_id, 'mode': mode, 'hash': content_hash}, headers={'referer': url}).text).cssselect('a[href$="mp4"]')
