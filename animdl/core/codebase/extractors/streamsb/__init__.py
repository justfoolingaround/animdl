...
def extract(session, url, **opts):
    ...
    sources = session.get(content_url + "sourcessx36/{}".format(PAYLOAD.format(binascii.hexlify(content_id.encode()).decode())), headers={'watchsb': 'streamsb'}).json().get('stream_data', {})
