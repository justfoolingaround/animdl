...
ON_NEW_TAB = regex.compile(r'openInNewTab\("([^"]+?)"\)')
ANIMEKAIZOKU_DDL = regex.compile(r'\("(\w+)",\d+,"(\w+)",(\d+),(\d+),\d+\)')
...
def get_links(session, quality, element_dicts, post_id, div_id):
    def fast_yield():
        for element_dict in element_dicts:
            num = regex.search(
                r"'([^']+?)'", element_dict.get('element', {}).get('onclick')).group(1)
            link = b64decode(ON_NEW_TAB.search(admin_ajax(session, {
                             'action': 'DDL', 'post_id': post_id, 'div_id': div_id, 'tab_id': 2, 'num': num, 'folder': 0}).text).group(1)).decode('utf-8')
...
def fetcher(session, url, check, match):
    ...
    content = htmlparser.fromstring(admin_ajax(session, {
                                    'action': 'DDL', 'post_id': post_id, 'div_id': div_id, 'tab_id': 2, 'folder': True}).text)
    selected = sorted(get_indexed(content.cssselect('.downloadbutton')),
                      key=lambda x: x.get('quality', 0), reverse=True)[0]

    num = regex.search(
        r"'([^']+?)'", selected.get('element', {}).get('onclick')).group(1)

    episodes = htmlparser.fromstring(admin_ajax(session, {
                                     'action': 'DDL', 'post_id': post_id, 'div_id': div_id, 'tab_id': 2, 'num': num, 'folder': 1}).text)
