...
from ...helper import (construct_site_based_regex, group_episodes,
                       parse_from_content)
...
def fetcher(session, url, check, match):
    ...
    for episode, streams in sorted(group_episodes((parse_from_content(regex_match, name_processor=lambda m: m.group(2), stream_url_processor=lambda m: (b64decode(m.group(1)).decode('utf-8'))) for regex_match in walk(session, post_id, div_id, 2, '', True))).items()):
        if check(episode):
