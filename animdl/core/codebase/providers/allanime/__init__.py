...
def iter_episodes(episode_dictionary: dict, anime_page_url: str):
    ...
    for type_of, episode_numbers in episode_dictionary.items():
        for episode in episode_numbers:
            episodes[int(episode) if episode.isdigit() else 0].append(("Episode {} [{}]".format(
                episode, type_of.upper()), anime_page_url + "/episodes/{}/{}".format(type_of, episode)))
    ...
    for title, url in content:
        for source_urls in SOURCE_URLS.finditer(session.get(url).text):
            ...
            if not content_uri.host:
                link_content = api_endpoint.join(content_uri.with_name(
                    content_uri.name + ".json").with_query(content_uri.query)).human_repr()
                ...
                for link in json_response:
                    ...
                    uri_ref = (stream_uri if stream_uri.host else api_endpoint.join(
                        stream_uri)).human_repr()
...
def fetcher(session, url: 'str', check, match):
    logging.warning(
        "This provider is slow at the cost of high amount of streams.")
    animepage_url = (ALLANIME + "anime/{}").format(match.group(1))

    api_endpoint = yarl.URL(session.get(
        ALLANIME + "getVersion").json().get('episodeIframeHead', ''))
