...
hard_urls = {
    "overflow": {
        "provider": "bestcdn",
        "streams": [
            (f"Overflow/Overflow {_:02d}.mp4", _) for _ in range(1, 9)
        ]
    },
    "jimihen-jimiko-wo-kaechau-jun-isei-kouyuu": {
        "provider": "bestcdn",
        "streams": [
            (f"Jimihen/Jimihen {_:02d}.mp4", _) for _ in range(1, 9)
        ]
    }
}
...
def yield_from_bestcdn(session, attributes):
    ...
    repl_string = REPL_REGEX.search(session.get("https://anfruete.github.io/play/env.js").text).group(1)
