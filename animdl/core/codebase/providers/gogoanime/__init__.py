...
def get_episode_list(session, anime_id):
    ...
    """
    episode_page = session.get(EPISODE_LOAD_AJAX, params={
                               'ep_start': '0', 'ep_end': '2000', 'id': anime_id, })
...
def get_stream_url(session, episode_page_url):
    ...
    response = session.get('https:%s' % streaming.replace(
        'streaming.php', 'download'), headers={'referer': "https:{}".format(streaming)})
