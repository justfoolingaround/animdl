...
def fetcher(session, url, check, match):
    ...
    for episode, episode_urls in sorted(get_from_url(session, url).items(), key=lambda x: x[0]):
        if check(episode):
            yield partial(lambda s, x: [*extract_stream_urls(s, x)], session, episode_urls), episode
