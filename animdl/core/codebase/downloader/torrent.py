...
def wrap_with_tqdm(session, btih, endpoint, torrent_name, log_level):
    ...
    progress_bar = tqdm(disable=log_level > 20, total=torrent.get('total_size', 0), unit='B', unit_divisor=1024, unit_scale=True, desc='qBittorrent / {}'.format(torrent_name), initial=torrent.get('total_downloaded', 0))
...
def download_torrent(_, magnet_uri, content_dir, torrent_name, endpoint, login_data, *, log_level=20):
    ...
    existent = session.post(endpoint + "/api/v2/torrents/properties", params={'hash': btih})

    if existent.status_code in [404, 400]:
        status = session.post(endpoint + "/api/v2/torrents/add", data={'urls': magnet_uri, 'savepath': content_dir.resolve().as_posix()}).text

        if status == 'Fails.':
            raise Exception("Could not add to the client: ({!r}, {!r})".format(magnet_uri, content_dir))
